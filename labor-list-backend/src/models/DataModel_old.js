...
class DataModel {
    ...
    static getHoje() {
        ...
        // Se for antes das 05:00 (ex: 01:00 da manhã), considera o dia anterior
        if (hora < 5 || (hora === 5 && minutos === 0)) {
    ...
    `);
        ...
        // 3. Obtém a data de referência ajustada pelo turno (antes das 05:00 = dia anterior)
        const hoje = DataModel.getHoje();

        // 4. CONSULTA MASSIVA: Dispara todas as consultas ao mesmo tempo (Promise.all)
...
ORDER BY c.nome
        ...
        // 5. PROCESSAMENTO DE DADOS ANTES DE ENVIAR PARA O REACT
        return {
            operadores: operadores.rows.map(op => {

                // --- Tratamento de Horas Extras agendadas ---
                let he = typeof op.overtime_dates === 'string' ? JSON.parse(op.overtime_dates) : (op.overtime_dates || []);
                // Oculta no Frontend HEs que já passaram de hoje (data ajustada pelo turno)
                he = he.filter(h => {
                    const d = typeof h === 'string' ? h : h.data;
                    return d >= hoje;
                });

                // --- Tratamento do histórico de faltas e saídas ---
                ...
                // ====================================================================
                // CORREÇÃO DINÂMICA DO STATUS "Absenteísmo" E "Saída Antecipada"
                // ====================================================================
                // Se o operador está com status 'Absenteísmo', verificamos se realmente
                // existe uma falta registrada na data de HOJE (ajustada pelo turno).
                // Se não existir, removemos o status para que ele não apareça mais como
                // ausente no dia seguinte (ex.: falta da madrugada pertence ao dia anterior).
                if (op.status_especial === 'Absenteísmo') {
                    const temFaltaHoje = hist_faltas.some(f => f.data === hoje);
                    if (!temFaltaHoje) {
                        op.status_especial = null;
                    }
                }

                // O mesmo tratamento é aplicado para 'Saída Antecipada', pois uma saída
                // registrada de madrugada também deve pertencer ao dia anterior.
                if (op.status_especial === 'Saída Antecipada') {
                    const temSaidaHoje = hist_saidas.some(s => s.data === hoje);
                    if (!temSaidaHoje) {
                        op.status_especial = null;
                        op.early_exit_time = null;  // limpa o horário de saída
                    }
                }

                // Retorna o operador com os campos JSON parseados e status corrigido
                return {
                    ...op,
    ...
    static async alterarStatus(dados) {
        ...
        try {
            ...
            if (novoStatus === 'Saída Antecipada') {
                // Determina a data real da saída considerando o turno do operador
                let dataSaida = hoje; // padrão: data ajustada atual

                // Se o horário da saída foi informado e for um horário noturno (ex.: após 17:48),
                // e o lançamento está sendo feito pela manhã (antes das 7:00),
                // assumimos que a saída é do dia anterior.
                if (earlyExit) {
                    ...
                    // Se o lançamento é feito após as 05:00 (turno já virou para o dia atual)
                    // e a saída foi à noite (após 17:48), a data correta é o dia anterior.
